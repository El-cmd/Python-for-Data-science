...
def main():
    try:
        ...
        years = [int(col) for col in df.columns if col != 'country' and int(col) <= 2050]
        france_population = [convert_population(france_row[str(year)]) / 1_000_000 for year in years]
        belgium_population = [convert_population(belgium_row[str(year)]) / 1_000_000 for year in years]
